...
$boq_today = mysqli_num_rows(mysqli_query($link,"SELECT * FROM boq WHERE DATE(FROM_UNIXTIME(date_added)) = CURDATE()"));
$customers_today = mysqli_num_rows(mysqli_query($link, "SELECT * FROM customers WHERE DATE(FROM_UNIXTIME(date_added)) = CURDATE()"));
$total_customers = mysqli_num_rows(mysqli_query($link, "SELECT * FROM customers ORDER BY customer_id"));
$work_orders_today = mysqli_num_rows(mysqli_query($link,"SELECT * FROM work_orders WHERE DATE(FROM_UNIXTIME(take_in_date)) = CURDATE()"));
$work_orders_tbi = mysqli_num_rows(mysqli_query($link,"SELECT * FROM work_orders WHERE work_order_status = 'To Be Inspected'"));
$work_orders_ip = mysqli_num_rows(mysqli_query($link,"SELECT * FROM work_orders WHERE work_order_status = 'In Progress'"));
$work_orders_oh = mysqli_num_rows(mysqli_query($link,"SELECT * FROM work_orders WHERE work_order_status = 'On Hold'"));
$work_orders_rfc = mysqli_num_rows(mysqli_query($link,"SELECT * FROM work_orders WHERE work_order_status = 'Ready For Collection'"));
$work_order_pu_today = mysqli_num_rows(mysqli_query($link,"SELECT * FROM work_order_notes WHERE DATE(FROM_UNIXTIME(date_added)) = CURDATE() AND note = 'Status set to Picked Up'"));
$work_order_notes_today = mysqli_num_rows(mysqli_query($link,"SELECT * FROM work_order_notes WHERE DATE(FROM_UNIXTIME(date_added)) = CURDATE()"));
// $sales_today = mysql_num_rows(mysql_query("SELECT * FROM computer_sales WHERE DATE(FROM_UNIXTIME(sale_date)) = CURDATE()"));
$total_technician = mysqli_num_rows(mysqli_query($link, "SELECT * FROM technician ORDER BY technician_code"));
...
// $desktop_stock = mysql_num_rows(mysql_query("SELECT * FROM computers WHERE status = 'stock' AND type = 'Desktop'"));
$product_stock = mysqli_num_rows(mysqli_query($link,"SELECT * FROM inventory WHERE status = 'stock'"));
// $computer_good_to_go = mysql_num_rows(mysql_query("SELECT * FROM computers WHERE status = 'goodtogo'"));

mysqli_query($link, "SELECT items,COUNT(*) as count FROM boq GROUP BY items ORDER BY count DESC");
mysqli_query($link, "SELECT length,COUNT(*) as count FROM boq GROUP BY length ORDER BY count DESC");
mysqli_query($link, "SELECT full_name,COUNT(*) as count FROM boq GROUP BY full_name ORDER BY count DESC");
mysqli_query($link, "SELECT quantity,COUNT(*) as count FROM boq GROUP BY quantity ORDER BY count DESC");
mysqli_query($link,"SELECT ward,COUNT(*) as count FROM customers GROUP BY ward ORDER BY count DESC");
mysqli_query($link,"SELECT referral,COUNT(*) as count FROM customers GROUP BY referral ORDER BY count DESC");
mysqli_query($link,"SELECT work_order_type,COUNT(*) as count FROM work_orders GROUP BY work_order_type ORDER BY count DESC");
mysqli_query($link,"SELECT make,COUNT(*) as count FROM work_orders GROUP BY make ORDER BY count DESC");
mysqli_query($link,"SELECT model,COUNT(*) as count FROM work_orders GROUP BY model ORDER BY count DESC");
mysqli_query($link,"SELECT type,COUNT(*) as count FROM work_orders GROUP BY type ORDER BY count DESC");
...
<div class="row">
	...
	<div class="col-md-offset-1 col-md-2">
		<div class="panel panel-default">
			<div class="panel-heading text-center"><i class='fa fa-users'></i></div>
	...
	<div class="col-md-2">
		<div class="panel panel-default">
			<div class="panel-heading text-center"><i class="fa fa-bell"></i></div>
			<div class="panel-body">
				<h2 class="text-center text-info"><?php echo $boq_today; ?></h2>
			</div>
			<div class="panel-footer">
				<div class="text-center"><small>New BOQ'S</small></div>
			</div>
		</div>
	</div>
	<div class="col-md-2">
		<div class="panel panel-default">
			...
			<div class="panel-footer">
				<div class="text-center"><small>New Orders</small></div>
			</div>
		</div>
	</div>
	<div class="col-md-2">
		<div class="panel panel-default">
			<div class="panel-heading text-center"><i class='fa fa-users'></i></div>
			<div class="panel-body">
			<h2 class="text-center text-danger"><?php echo $total_customers; ?></h2>
			</div>
			<div class="panel-footer">
				<div class="text-center"><small>Total Customers</small></div>
			</div>
		</div>
	</div>
	<div class="col-md-2">
		<div class="panel panel-default">
			<div class="panel-heading text-center"><i class='fa fa-user-tie'></i></div>
			<div class="panel-body">
				<h2 class="text-center text-danger"><?= $total_technician; ?></h2>
			</div>
			<div class="panel-footer">
				<div class="text-center"><small>Total Technicians</small></div>
			</div>
		</div>
	</div>
</div>
			...
			<table class="table">
				<thead>
					...
					<tbody>
						...
						$sql = mysqli_query($link,"SELECT * FROM users, customers, work_orders
						...
						while ($row = mysqli_fetch_array($sql)) {
							$type = $row['type'];
							$items = ucwords($row['product']);
							$quantity = ucwords($row['quantity']);
							$username = ucwords($row['Username']);
							$first_name = $row['first_name'];
							$work_duration = $row['work_order_duration'];
							$last_name = $row['last_name'];
							$customer_id = $row['customer_id'];
			...
			<div class="panel-heading text-center"><i class='fa fa-edit'></i> WorkOrder Notes Made Today</div>
				<table class="table">
					<tbody>
						...
						$sql = mysqli_query($link,"SELECT * FROM users, work_order_notes
